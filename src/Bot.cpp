...
const std::map<Piece, int> Bot::piece_scores = {{NAP, 50}, {PAWN, 350}, {KNIGHT, 500}, {BISHOP, 600},{ROOK, 700}, {QUEEN, 1000}, {KING, 0}};
...
const float Bot::protectedPiece = 3.5;
const float Bot::attackedPiece = 2;
const float Bot::protected_attackedPiece = 1.25;
const float Bot::attackedNAP = 1.5;
const int Bot::kingInCheck = 750;
  ...
  for(int i = 0; i < 8; i++) {
    for(int j = 0; j < 8; j++) {
      ...
      // process slots attacked by enemy
        ...
        if(PieceHandlers::getType(table.table[i][j]) == KING
            && PieceHandlers::getColor(table.table[i][j] == sideToMove))
            score -= (int)(Bot::kingInCheck);
          // my atacked piece
          else if(PieceHandlers::getType(table.table[i][j]) != NAP
            && PieceHandlers::getColor(table.table[i][j] == sideToMove))
            score -= (int)(Bot::attackedPiece * get_score_by_position(PieceHandlers::getType(table.table[i][j]), i, j));
